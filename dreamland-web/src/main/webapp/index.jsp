...
<head>
    ...
    <style>
        ...
        a:hover{
            cursor: url('./images/UpArrow.cur'),auto;
        }
        .stats-buttons  a{
        ...
        a:-webkit-any-link {
            cursor: url('./images/Hand.ani'),auto;
    ...
    <div id="content" class="row-fluid">
        ...
        <div class="col-md-3" style="position:absolute;top:0px;left: 880px;width: 268px;">
            <div style="background-color: white;width: 250px;height: 440px">
                <iframe name="weather_inc" src="http://i.tianqi.com/index.php?c=code&id=55&py=minhou" width="250" height="440" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>
            </div>
        </div>
...
<script language=javascript>
                    ...
                    if(list!=null && list!=""){
                        $(list).each(function () {
                            ...
                            var chtml = "<div class='comment-show'>"+
                                ...
                                "</div> <div class='date-dz'><span class='date-dz-left pull-left comment-time'>"+this.commTime+"</span>"+
    ...
    function _comment(content_id,uid,cuid) {
        ...
        if(oSize.replace(/(^\s*)|(\s*$)/g, "") != ''){
            ...
            $.ajax({
                type:'post',
                url:'/comment',
                data: {"content_id":content_id,"uid":uid,"oSize":oSize,"comment_time":myDate},
                dataType:'json',
                success:function(data){
    ...
    function reply_up(id) {
        ...
        if($("#change_color_"+id).is('.date-dz-z-click')) {
            num--;
            $("#change_color_" + id).removeClass('date-dz-z-click');
    ...
    function comment_pl(content_id,comment_id,fid,by_id,cuid) {
        ...
        if(oHfVal.replace(/^ +| +$/g,'') == '' || oHfVal == oAllVal){
            ...
            $.ajax({
                type:'post',
                url:'/comment_child',
                data: {"content_id":content_id,"uid":'${user.id}',"oSize":oHfVal,"comment_time":myDate,"by_id":by_id,"id":fid},
                dataType:'json',
                success:function(data){
                    var comm_data =  data["data"];
                    //alert(comm_data);
